...
{
    [SerializeField] private WeaponsItems weaponType;

    [SerializeField] private WeaponTake weaponPlayer;
...
private void Start() {
      GameObject player;
        ...
        weaponPlayer = player.GetComponent<WeaponTake>();
}
    /// <summary>
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (rayresult.Count > 0) {
            if (rayresult[0].gameObject.name == "Cell Equiped") {
                // if (sourceCell.transform.childCount > 0){
                    weaponPlayer.AttachWeaponOnPlayer(name);
                // }
          
                // weaponPlayer.AttachWeaponOnPlayer(rayresult[0].gameObject);
                // print(eventData);
            }
            else {
