	...
	void Update() {
		...
		foreach(Collider tank in colliders) {
			...
			if (distance < nearestDistance && distance > 1) {
				...
				canon.transform.LookAt (nearestTank.transform);
			}
		}

//		Vector3 fwd = canon.transform.TransformDirection (new Vector3(0, 0, 2));
//		Debug.Log(fwd)
//		if (Physics.Raycast (canon.transform.position, fwd, out raycastHit, 50)) {
//			fireGun = true;
